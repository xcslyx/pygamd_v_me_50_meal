...
class RMSDCalculatorAgent:
    def __init__(self, path, ref: str=None, cal_class_rmsd: str=None, domain: str | None=None, balance_cut: str | None=None):
        """
        用于计算 RMSD.
        ...
        self.ref = ref
        assert self.ref, "Error! Please input the reference structure XML file including a single reference structure!"
        self.init_pos = self.get_init_pos(self.ref)  # 读取参考结构的初始位置

        self.cal_class_rmsd = [self.data.mol_class_list[i] for i in list(map(lambda x: int(x) - 1, cal_class_rmsd.split(',')))]
    ...
    def cal_rmsd(self, chain_xyz):
        ...
        assert aa_num == len(chain_xyz), "Error! Length of chain is not equal to that of initial chain.!"
    ...
    def calculate(self):
        if not self.balance_cut:
            ...
            if end == -1:
                chain_files = os.listdir(self.chain_path)[start - 1:]
            else:
